...
public class NoteController {
	...
	public ResponseEntity<?> createNote(@RequestBody Note note) {
		System.out.println("Within Note Controller " + note.toString());
	...
	@DeleteMapping("/api/v1/note/delete/{userId}/{id}")
	public ResponseEntity<?> deleteNote(@PathVariable String userId, @PathVariable Integer id) {
		System.out.println("&&&&& NoteController deleteNote "+userId+ "  "+id);
	...
	public ResponseEntity<?> updateNote(@RequestBody Note note, @PathVariable int id, @PathVariable String userId) {
		...
		System.out.println("$$$$$  NoteController updateNote "+note.toString());
		try {
	...
	public ResponseEntity<?> getAllNotesByUserId(@PathVariable String userId) {
		...
		System.out.println("Within Note Controller getAllNotesBy UserId " + userId);
	...
	public ResponseEntity<?> getNoteByNoteId(@PathVariable String userId, @PathVariable String noteId) {
		
		System.out.println("$$$$ Within NoteController getNoteByNoteId userId - "+userId +" - noteId- "+noteId);
		int noteIds = Integer.parseInt(noteId);
		try {
			Note note = noteService.getNoteByNoteId(userId, noteIds);
			System.out.println("$$$$ Within NoteController getNoteByNoteId note- "+note.toString());
			return new ResponseEntity<Note>(note, HttpStatus.OK);
